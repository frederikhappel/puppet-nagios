# == Define: nagios::core::resource::eventhandler
#
...
define nagios::core::resource::eventhandler (
  ...
  if is_puppet_source($source) {
    ...
    if $script_name == $name {
      ...
      $checkscript = "${nagios::core::params::eventhandlerdir}/${source_parts[-1]}"
    } else {
      $checkscript = "${nagios::core::params::eventhandlerdir}/${script_name}"
    }
    File[$checkscript] {
      ensure => $ensure,
      source => $source,
    ...
    if is_absolute_path($source) {
      ...
      $checkscript = "${nagios::core::params::eventhandlerdir}/${source}"
    }
  } elsif ($content != undef) {
  ...
  if $commands != undef {
    # install check command configurations
    file {
      "${nagios::core::params::resourcesdir}/eventhandler_${name}.cfg" :
        ensure => $ensure,
